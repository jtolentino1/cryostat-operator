...
type cryostatTestInput struct {
	reconciler controller.CommonReconciler
...
func (c *controllerTest) commonJustBeforeEach(t *cryostatTestInput) {
	...
	t.reconciler, err = c.constructorFunc(t.newReconcilerConfig(s, t.Client))
	Expect(err).ToNot(HaveOccurred())
}
...
func (t *cryostatTestInput) newReconcilerConfig(scheme *runtime.Scheme, client ctrlclient.Client) *controller.ReconcilerConfig {
	logger := zap.New()
...
func resourceChecks() []resourceCheck {
	return []resourceCheck{
		...
		{(*cryostatTestInput).expectCoreIngessNetworkPolicy, "core networkpolicy"},
		{(*cryostatTestInput).expectMainDeployment, "main deployment"},
		{(*cryostatTestInput).expectDatabaseDeployment, "database deployment"},
		{(*cryostatTestInput).expectDatabaseIngressNetworkPolicy, "database networkpolicy"},
...
func (c *controllerTest) commonTests() {
	...
	Describe("reconciling a request in OpenShift", func() {
		...
		Context("with egress networkpolicies enabled", func() {
			var cr *model.CryostatInstance
			BeforeEach(func() {
				cr = t.NewCryostat()
				enabled := true
				...
				t.objs = append(t.objs, cr.Object, endpoints)
			})
			JustBeforeEach(func() {
				t.reconcileCryostatFully()
			})
			It("should create cryostat networkpolicy", func() {
				t.checkNetworkPolicy(t.NewCryostatEgressNetworkPolicy())
			})
		})
		Context("with report generator service", func() {
			...
			Context("with cert-manager disabled", func() {
				...
				It("should configure deployment appropriately", func() {
					...
					t.checkNetworkPolicy(t.NewReportsIngressNetworkPolicy())
				})
			})
		...
		Context("with an existing DB PVC", func() {
			...
			Context("that fails to update", func() {
				JustBeforeEach(func() {
					...
					origClient := t.reconciler.GetConfig().Client
					t.reconciler.GetConfig().Client = test.NewClientWithUpdateError(origClient, invalidErr, func(obj ctrlclient.Object) bool {
				...
				It("should emit a PersistentVolumeClaimInvalid event", func() {
					recorder := t.reconciler.GetConfig().EventRecorder.(*record.FakeRecorder)
					var eventMsg string
		...
		Context("cert-manager missing", func() {
			JustBeforeEach(func() {
				...
				t.reconciler.GetConfig().RESTMapper = meta.NewDefaultRESTMapper([]schema.GroupVersion{})
			})
		...
		Context("with an existing Cryostat CR", func() {
			...
			JustBeforeEach(func() {
				...
				reconciler, err := c.constructorFunc(config)
				Expect(err).ToNot(HaveOccurred())
				otherInput.reconciler = reconciler
		...
		Context("with an outdated cert-manager", func() {
			...
			JustBeforeEach(func() {
				...
				origClient := t.reconciler.GetConfig().Client
				t.reconciler.GetConfig().Client = test.NewClientWithCreateError(origClient, invalidErr, func(obj ctrlclient.Object) bool {
	...
	Describe("setting up with manager", func() {
		...
		JustBeforeEach(func() {
			...
			Expect(err).ToNot(HaveOccurred())
			t.reconciler.SetupWithManager(mgr)
		})
...
func (t *cryostatTestInput) expectCoreIngessNetworkPolicy() {
	t.checkNetworkPolicy(t.NewCryostatIngressNetworkPolicy())
}

func (t *cryostatTestInput) expectDatabaseIngressNetworkPolicy() {
	t.checkNetworkPolicy(t.NewDatabaseIngressNetworkPolicy())
}

func (t *cryostatTestInput) expectStorageIngressNetworkPolicy() {
	t.checkNetworkPolicy(t.NewStorageIngressNetworkPolicy())
}

func (t *cryostatTestInput) expectReportsIngressNetworkPolicy() {
	t.checkNetworkPolicy(t.NewReportsIngressNetworkPolicy())
}
...
func (t *cryostatTestInput) checkNetworkPolicySpec(policy *netv1.NetworkPolicy, expected *netv1.NetworkPolicy) {
	...
	Expect(policy.Spec.PolicyTypes).To(Equal(expected.Spec.PolicyTypes))
	Expect(policy.Spec.PodSelector).To(Equal(expected.Spec.PodSelector))
	Expect(policy.Spec.Ingress).To(Equal(expected.Spec.Ingress))
	Expect(policy.Spec.Egress).To(Equal(expected.Spec.Egress))
}
...
func (t *cryostatTestInput) reconcileWithName(name string) (reconcile.Result, error) {
	...
	return t.reconciler.Reconcile(context.Background(), req)
}
